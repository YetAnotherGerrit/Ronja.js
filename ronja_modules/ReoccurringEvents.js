    ...
    hookForEventStart: async function (
        ...
        if (myRegResult) {
            ...
            newGuildScheduledEvent.guild.scheduledEvents.create({
                ...
                scheduledStartTime: DateTime.fromJSDate(
                    newGuildScheduledEvent.scheduledStartAt
                )
                    .plus({ days: pDays })
                    .toJSDate(),
                scheduledEndTime: newGuildScheduledEvent.scheduledEndTimestamp
                    ? DateTime.fromJSDate(newGuildScheduledEvent.scheduledEndAt)
                          .plus({ days: pDays })
                          .toJSDate()
                    : newGuildScheduledEvent.scheduledEndTimestamp, // Optional, but not for EXTERNAL
                ...
                image: newGuildScheduledEvent.coverImageURL()
                    ? newGuildScheduledEvent.coverImageURL() + "?size=1024"
                    : null, // Optional
