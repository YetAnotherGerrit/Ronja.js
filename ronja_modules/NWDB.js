...
const myNWDB = {
    ...
    myGetServerStatus: async function (lng, getServer) {
        ...
        let regex = new RegExp(
            `\\[(\\d+),(\\d+),(\\d+),\\d+,\\\\\\"${getServer}\\\\\\",\\\\\\"Vanaheim Zeta\\\\\\",\\\\\\"eu-central-1\\\\\\",\\d+,\\\\\\"(\\w+)`
        );
        ...
        if (serverStatus == "ACTIVE") {
            resultString =
                ...
                ": " +
                serverPlayers +
                this.l(lng, " active players");
        } else {
            resultString =
                getServer +
                ": " +
                this.l(lng, serverStatus) +
                " (" +
                serverPlayers +
                "/" +
                serverLimit +
        ...
        if (serverQueue > 0) {
            resultString =
                resultString + this.l(lng, ", %s in the queue", serverQueue);
        }
    ...
    hookForCron: function () {
        if (!this.cfg.newWorldChannel || !this.cfg.newWorldServer) {
            console.info(
                "INFO: newWorldChannel or newWorldServer not set in config file, disabling New-World queries!"
            );
            return [];
        }
        return [
            {
                ...
                action: () => {
                    this.client.channels
                        .fetch(this.cfg.newWorldChannel)
                        .then((c) => {
                            this.myGetServerStatus(
                                c.guild.preferredLocale,
                                this.cfg.newWorldServer
                            )
                                .then((res) => {
                                    if (res != "") {
                                        c.setTopic(res);
