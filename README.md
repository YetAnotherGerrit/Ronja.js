...
- Optional [IGDB](https://www.igdb.com/) integration to reduce duplicate
  ...
  the top 10 also show the (top) game's cover art. Games tracked before IGDB
  was enabled are matched gradually in the background, asking the server owner
  via DM when a match isn't certain, and game data is kept up to date. Configured with a Twitch/IGDB client ID and secret via
