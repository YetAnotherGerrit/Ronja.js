...
`hookForSettingOptions(name, locale)` is likewise not fanned out from an event: `/settings` (`ronja_modules/Settings.js`) calls it on every module to get the choices of a `multiselect` setting (shown as a multi-select menu, stored as a comma-separated list of the picked values) and uses the first non-empty answer. `IGDB.js` answers it for `gameCardDetails`, which picks the details `core/gameCard.js` shows on the game card.
